...
func NewWithEAGI(r io.Reader, w io.Writer, eagi io.Reader) *AGI {
	...
	slog.Info("New AGI Session Created ", slog.Int("Id", a.id))

	s := bufio.NewScanner(a.r)
	slog.Info("Session Variables : ")
	var key string
	var value string
	for s.Scan() {
		if s.Text() == "" {
		...
		if len(terms) == 2 {
			...
			slog.Info("", slog.String(key, value))
...
func NewConn(conn net.Conn) *AGI {
	slog.Info("New AGI Connection Created")
	a := New(conn, conn)
	a.conn = conn
	return a
}
...
func toEpoch(when time.Time) string {
	return strconv.Itoa(int(when.Unix()))
}
